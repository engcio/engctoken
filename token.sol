...
contract MyAdvancedToken is owned, TokenERC20 {
    ...
    function transferMultiple(address[] tos , uint256[] values,uint count) public returns(bool) {
        uint256 total = 0;
        ...
        for(i=0;i<count;i++){
            balanceOf[msg.sender] = SafeMath.sub(balanceOf[msg.sender],values[i]);
            balanceOf[tos[i]] = SafeMath.add(balanceOf[tos[i]], values[i]);
            Transfer(msg.sender, tos[i], values[i]);
        }
